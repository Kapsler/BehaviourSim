...
void Map::GenerateFromImage(float screenWidth, float screenHeight, const sf::Image& mapImage)
{
	...
	for (int i = 0; i < rows; ++i)
	{
		...
		for (int j = 0; j < columns; ++j)
		{
			...
			{
				hexDatTmp->terrain = floorDifficulty;
				tmp->setFillColor(floorColor);
			}
			else if(currentColor == wallColor)
			{
				hexDatTmp->terrain = wallDifficulty;
				tmp->setFillColor(wallColor);
			}
			else if(currentColor == doorColor)
			{
				hexDatTmp->terrain = doorDifficulty;
				tmp->setFillColor(doorColor);
			}
			else
			{
